...
## 性能上限：一键 chip 为什么追不上官方

**一键改写的质量达不到 Apple 官方 Writing Tools 的水平，而且没法通过调优追上。**
想要官方水准，用 **✨ chip** —— 那是手动模式，走的是官方那套。

### 为什么

官方 Writing Tools 不是拿通用模型写 prompt。它加载**任务专属的 LoRA 适配器**——校对一个、
摘要一个、语气一个——运行时按需换入。然后通过 `NSWritingToolsCoordinator` 把
**`(范围, 替换文本)` 对**交回 App，直接编辑已有的文本存储，而不是返回一整段文字。

公开的 FoundationModels API 两样都不给：你拿到的是通用的约 3B 基座模型和整段字符串响应。
官方那些适配器是私有系统资源，`SystemLanguageModel.Adapter(name:)` 加载的是**你自己提供**的
适配器，不是 Apple 的。
...
| 动作 | 在真实聊天消息上的表现 |
|---|---|
| 校对 | 可靠。这是基座模型的天然模式，做得很好。 |
| 专业 | 必须加缰绳。不加的话，一句话的聊天消息会变成 2.4 倍长的信函模板——`Hi [Recipient's Name] … Best regards, [Your Name]`。 |
| 友好 | 同样的缰绳，同样的原因；它爱在结尾加「Thanks for your understanding!」。 |
| 改写 | 结果常常和「校对」难以区分，增量价值低。 |
| 简洁 | **不可靠。** 约一半情况下把原文原样交回。而且把 prompt 写强**反而更糟**：加上「结果必须明显更短」和字符数预算之后，模型干脆完全放弃编辑、直接复制输入。所以这里保留了温和版的 prompt。 |
| 翻译 | 根本没用大模型——它会编。「明天」被翻成了「Wednesday」。翻译走 `Translation.framework`。 |
...
3. **合理长度上限**。每个动作声明结果最多能比原文长多少，超出就丢弃，绝不粘进你的文档。

如果结果和原文完全一致，悬浮球会直接告诉你「没有可改的地方」，而不是把你自己的话原样贴回去。

### 那自己训一个适配器行不行

技术上可以。API 是公开的，Apple 也提供训练 rank-32 LoRA 的 Python 工具包
（Apple silicon + ≥32GB 内存，或 Linux GPU 机器）。但这里不值得，两个原因：

- **适配器锁死基座模型版本**——一个系统模型版本对应一个适配器，无一例外。
  macOS 一更新就可能失效，在你重新训练并发版之前，所有用户的 App 都是坏的。
- **它补不上真正的差距。** 和官方的差别不只是模型，更是**传输格式**：官方按范围原地编辑，
  我们是整块替换。换适配器改变不了这一点。

所以分工是诚实的：**chip 是便利路径，✨ chip 是质量路径。**
