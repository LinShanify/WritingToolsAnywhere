...
**内联部分刻意只做一件事。** 早先版本有五个一键改写——友好、专业、简洁等等。
在真实聊天消息上实测，它们的表现从「多余」到「坏掉」不等，而且原因是
prompt 调不动的（见[性能上限](#性能上限内联为什么追不上官方)）。
凡是超出「把语法弄对」的需求，都该交给 Apple 的面板——它才有对应的模型。
...
| 动作 | 表现 | 结论 |
|---|---|---|
| 校对 | 可靠——这是基座模型的天然模式 | **保留** |
| 改写 | 结果常和「校对」难以区分 | 并入校对 |
| 专业 | 不加缰绳时，一句话的消息会变成 2.4 倍长的信函模板：`Hi [Recipient's Name] … Best regards, [Your Name]` | 移除 |
| 友好 | 同样问题，还爱在结尾加「Thanks for your understanding!」 | 移除 |
| 简洁 | 约一半概率把原文原样交回。而且把 prompt 写强**反而更糟**：加上「结果必须明显更短」和字符数预算后，模型干脆放弃编辑、直接复制输入 | 移除 |

翻译也用大模型试过，然后被否掉了——因为它**会编**。
「明天」被翻成了「Wednesday」，一个没翻的 "someone" 直接留在中文句子里。
`Translation.framework` 犯的是普通的选词错误，不是笃定的捏造——
...
3. **合理长度上限**。校对结果超过原文约 1.6 倍，那就是「续写」而不是「修正」，直接丢弃。
